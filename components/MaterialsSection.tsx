...
function MaterialRow({
  ...
  async function save() {
    ...
    const result = await updateMaterial(material.id, { quantity_ordered, quantity_used, unit_cost });
    if (result.error) {
      setError(result.error);
    } else {
      onUpdate(material.id, { quantity_ordered, quantity_used, unit_cost });
      setEditing(false);
    }
    setSaving(false);
  }
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <div className="flex-1">
              ...
              <input
                ...
                step="any"
                value={orderedVal}
                onChange={(e) => setOrderedVal(e.target.value)}
                placeholder="0"
